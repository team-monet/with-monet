...
- Stig's **product context and Aria lens**: product context stays separate from team ways of working and personal preferences; current-circle context governs while sibling projects are analogy only; missing/stale/disputed context is refreshed only when decision-relevant; Stig itself shapes and gets user ratification for material product-work contracts, keeps contract drafting, issue creation, and implementation as separate authorization boundaries, calibrates technical evidence inside the ratified contract with explicit supported contracts and safety invariants taking precedence over stored risk preferences, and applies a product-outcome verdict within the existing combined gate — no extra Aria agent or review cycle, in team or lead-only mode,
- Stig's **experience-curve loop**: behavior-changing feedback becomes a scoped lesson with applicability boundaries and an expected behavior change, then is applied before analogous decisions and confirmed, narrowed, or corrected when outcome evidence appears; retrieval alone is not treated as learning, and one-off ticket state never becomes Monet memory,
...
4. **Product judgment survives install and update.** Read back the installed Stig block and confirm the same Stig-owned Aria lens is present in team and lead-only modes: a fresh repo establishes product context only when a product decision needs it; routine technical work skips product ceremony; foreign product context cannot govern; the user ratifies priority and scope; drafting does not authorize issue creation or implementation; explicit supported contracts and safety invariants override stored risk preferences; a contract-changing finding returns to the user; a technically complete but product-incomplete result is `Product Gap`; and scoped corrections affect analogous later decisions without over-inheriting across counterexamples or projects.
