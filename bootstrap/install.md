...
Merge into the host's **user-level** MCP config **without clobbering existing servers** — read it, add `monet`, write it back, and back up the file first. Verify it comes up: `monet start` logs `semantic embeddings ready (MiniLM, 384-dim)` then `Monet started` — that's the "it's up" signal on any version. On `@team-monet/monet` **≥ 0.2.0** it also prints the active project circle (`Circle: <name>`), confirming per-project isolation. Tools it exposes: `agent_context`, `memory_store`, `memory_search`, `memory_overview`, `memory_gather`, `memory_fetch`, `memory_synthesize`, `memory_checkpoint`, `memory_flag_contradiction`, `memory_resolve` — plus `memory_list` and `memory_reassign_circle` on **≥ 0.2.0** (the consolidation flow uses these; older runtimes lack them, so fall back as in [`consolidate-memory.md`](consolidate-memory.md)) — plus `memory_detach` on **≥ 0.3.0** (splits observations out of a concept for wrong-merge repair — or, given `destConceptId`, folds them into another concept; detaching ALL observations with `destConceptId` consolidates the source concept away, carrying its identity as an alias on the keeper so references keep resolving; on those versions `memory_fetch` returns observation entries as `{id, content}` and pages with `observationsOffset`, `memory_store` accepts `resolution: "forceNew"`/`attachTo` for bulk import and directed attach, and `memory_overview` surfaces `possibleDuplicates`) — plus `memory_circle_manage` on **≥ 0.5.0** (circle lifecycle: rename/merge/archive with stable aliases; `memory_reassign_circle` also gains `ids` batching and `resolution: "auto"|"forceNew"` there; recall is store-wide on ≥ 0.4.0, so reorganizing circles never breaks findability — the interactive curation ritual lives in [`curate-memory.md`](curate-memory.md)) — plus the following on **≥ 0.6.0**: (a) `memory_fetch` responses now include `lastConfirmedAt` (Unix ms of the last evidence-based confirmation — used as the staleness signal); (b) `agent_context` `staleConcepts` is capped at 20, sorted stalest-first, and staleness now derives from `lastConfirmedAt` rather than last-mutated-at ("not confirmed lately", not "not edited lately"); (c) `memory_resolve` gains a second verdict family — **duplicate-pair dismissal**: pass `conceptAId` + `conceptBId` (omit `contradictionId`/`decision`) to assert two concepts are NOT duplicates; the pair is cleared from `memory_overview.possibleDuplicates`; re-dismissing a pair where no live edge exists is idempotent and returns `rowsUpdated: 0`; passing `decision` or `body` alongside `conceptAId`+`conceptBId` is rejected with a clean error; (d) `memory_resolve` contradiction path: retrying an already-closed contradiction now returns a non-error `alreadyClosed: true` response with zero mutations (previously any retry was undefined behavior).
