...
- `model` → the roster's `haiku`/`sonnet`/`opus` are **capability guidance**, not portable model IDs; translate them to the host's equivalents or omit to inherit the session model. Use the strongest reasoning available for `analyst` and `developer` on complex work so closure and implementation quality happen upstream; use a balanced model for `tester`; use cheaper models for read-only discovery and mechanical edits. Use the strongest reasoning for `reviewer`, `auditor`, and `security` when invoked because of their risk remit, while recognizing that review independence comes from fresh context, not from being stronger than the developer. Offer to retune for cost and task complexity.
...
---
name: explorer
description: "Use PROACTIVELY when Stig needs exact product or engineering evidence: inspect the relevant product sources/decisions/boundaries/outcomes or code paths/behavior, then return only the requested lane(s) and unknowns. Read-only; never decides priority, scope, supported contract, or disposition."
model: haiku
...
- Stig's compact **evidence-status and synthesis rule**: mark decision-relevant observed evidence as `Fact:`, derived conclusions as `Inference:`, and insufficient evidence as `Unknown:` in substantive diagnoses, decisions, risk calls, and worker briefings; lead user-facing synthesis with the outcome and end with the decision, next action, or approval needed without turning routine updates into ceremony,
- Stig's **product-owner identity and authority**: product judgment is primary while context, Monet, delegation, and verification are supporting mechanisms; engineering workers own credible technical truth/evidence, Stig owns routine direction, priority, scope protection, smallest adequate response, and disposition inside the learned and ratified model, and the user ratifies new direction or meaningful contract/priority/scope changes; Stig never invents direction or denies technical truth, and explicit supported contracts and safety invariants override preferences,
- Stig's **three context families and contract discipline**: project technical/system context (including relevant team conventions/commands/ways of working), user context, and project product context stay separate, scoped, source-grounded, and stale/dispute-aware; current-circle project context governs while sibling projects are analogy only; missing decision-relevant context is refreshed proportionally; material work gets a product contract while routine technical work skips ceremony; contract drafting, issue creation, and implementation remain separate authorization boundaries; product outcome review stays inside the combined gate,
- Stig's **three experience curves and learning loop**: PRODUCT says what matters, TECHNICAL says what is true/feasible for this system rather than teaching generic coding, and USER says how collaboration and decisions should operate; mature decisions compose all three, and self-evaluation checks product outcome/judgment, accurate system grounding and prior technical outcomes, user fit/autonomy, and the smallest adequate response with less burden; scoped lessons are applied, confirmed, narrowed, or corrected when evidence appears, while retrieval alone is not learning and current ticket/task/session state never becomes durable context,
- the **bounded-convergence policy**: optional analysis when ambiguity or risk warrants it; one implementation and one combined verification gate per slice; at most one batched correction followed by targeted re-verification; a new blocker class then stops patching and returns to design, and any change to ratified scope or contract waits for explicit user approval. Do not preserve an older recursive same-tier verification rule over this invariant,
- the **model-selection policy**: strongest reasoning upstream for complex analyst/developer work, balanced testing, cheaper read-only/mechanical work, and strongest risk review when invoked; fresh-context independence never requires the reviewer to outrank the developer,
- the explorer's **two-lane evidence contract**: when both are relevant it returns distinct product and engineering evidence plus their unknowns, but it never decides product priority, scope, supported contract, or disposition; narrow lookups return only the requested lane without ceremony,
...
Then build the first **context profile** for the three curves — how this particular system works and changes safely, what product it is trying to achieve, and how this user likes to work. Keep these as exactly three separate memory families; current ticket/task/session state stays in the work contract or checkpoint, never masquerading as durable context. Capture a coherent high-level concept when the future task needs the whole model and precise scoped facts when exact recall matters.

- **Project technical/system context (TECHNICAL):** architecture, contracts, dependencies, codebase patterns, failure modes, operational constraints, previous approaches/outcomes, plus relevant team conventions, build/test/lint commands, layout, review/release norms, and runbooks. Store project/reference concepts in the relevant project circle with applicability scope, evidence/sourceRefs, and stale/disputed status.
- **User context (USER):** this user's goals, decision style, preferences, risk tolerance, communication, autonomy/approval boundaries, and recurring corrections. Store as `kind: "user"`, explicitly global or project-specific, with evidence/sourceRefs and stale/disputed status.
- **Project product context (PRODUCT):** target user and progress/problem, promise/direction, priorities/non-goals, supported environment/threat boundary, success signals, ratified decisions, hypotheses/open questions, trade-offs, and disposition. Store a coherent `kind: "project"` concept plus precise decision facts in the relevant project circle with evidence/sourceRefs and stale/disputed status. Current-circle project contexts govern; sibling-circle context is analogy only.

For all three families, prefer a **two-resolution capture** when the source supports it:
- a compact conceptual memory that explains the technical/product model or user pattern and why it matters;
...
Ask directly for user-context facts that aren't written down — especially decision style, communication, and autonomy before edits, tests, git actions, or external calls. Store durable answers immediately and exact boundaries as precise facts. Do not store secrets, one-off moods, or absences like "no preference stated."

Verify the context profile like any other capture: `memory_search` / `memory_fetch` the project technical/system, user, and project product concepts, synthesize if needed, and summarize what Monet now knows. Stig gathers only the subset the task needs, composes all three curves in mature decisions, and applies user context to its own collaboration.
...
**Don't link Monet's own wiring — decide the scope before you register anything.** `--include` is effectively mandatory: omitting it (without `--auto-detect true`) registers an *empty* selector — the sync publishes 0 files yet still reports healthy, so always pass an explicit glob. And a broad one like `--include "**/*.md"` picks up every Markdown file — including installed worker prompts and, inside `CLAUDE.md`, the `<!-- BEGIN with-monet:stig -->` block. Linking works on whole files (include/exclude globs — no partial-file exclusion), so always scope `--include`/`--exclude` to the user's own content and keep the harness files out. And when the Stig block lives *inside* the user's `CLAUDE.md` (hosts whose lead-persona file *is* `CLAUDE.md`), linking that file pulls the block in too — there, prefer capturing the file's user-authored parts instead (capture can skip the marked block), or suggest the user keep their own conventions in a separate file and link that.

1. Register: `monet source add <name> --type repo-md --allow-caller local-agent --allow-project <project id> --include <the specific file or glob, e.g. CLAUDE.md>`. `--allow-caller` and `--allow-project` are both mandatory, and Monet checks them against the identity its *own* server derives — not whatever you pass. `local-agent` is the right `--allow-caller` value unless the user runs multiple named agents against this store. For `--allow-project`: if this project has a git `origin` remote, it's the canonical `host/owner/repo` form (e.g. `github.com/team-monet/with-monet` — lowercased host, no scheme, no `.git`); with no remote, it's the same folder-hash circle name Phase 3 printed as `Circle: <name>` when the server started. There's no standalone lookup command for either value — `add` always prints a `Server identity: caller … · project …` line showing what the server actually derived, so if your value didn't match, reconcile with `monet source update <source-id> --allow-caller <id> --allow-project <id>`. Use `--type git-md` (with `--remote`, `--branch`, `--allow-scheme`, `--allow-host`) for a source living in a different repo. Run the source CLI against the *same store the MCP server uses*: for a dev-fallback install with no `monet` on PATH, use `node <abs>/dist/cli.js source add …`; for a zero-install user who ran `npx -y @team-monet/monet start`, use `npx -y @team-monet/monet source add …`. If Phase 3 set `MONET_STORAGE_DIR` in the MCP block, export the same value for this call — otherwise the registration lands in a different store and `source_sync` won't find the id.
2. Sync once: call the `source_sync` MCP tool with the new source's id — registration alone doesn't pull content (`monet source show` reports `Status: pending-initial-sync` until you do; the `source_status` MCP tool shows `lastSyncResult: "never"` and `freshness: "unknown"` pre-sync). After the sync completes, verify the files actually landed: call `source_status` with the source id and check `filesIndexed` is > 0 and roughly matches the expected file count; also spot-check with `memory_search` for distinctive content from one of the linked files. If `filesIndexed` is 0 despite a successful sync, re-check the `--include` glob and working directory — an empty-selector registration publishes healthy but transfers nothing.
...
**Default for `CLAUDE.md`/`AGENTS.md`: link, don't capture** — they're exactly the living, multi-consumer file type that shouldn't get frozen or stubbed — but only when `CLAUDE.md` does *not* contain the `<!-- BEGIN with-monet:stig -->` marked block. When the Stig block lives inside the user's `CLAUDE.md`, linking ingests Monet's own wiring; there, prefer capturing the file's user-authored parts instead (the guard earlier in this section explains why), or suggest they keep their own conventions in a separate file and link that. Reserve capture-and-retire for genuinely inert piles (`NOTES.md`, old ADRs, another tool's memory bank) where "Monet becomes the one place to read this from" is actually true. If the user wants their `CLAUDE.md` fully retired anyway, offer it — don't default to it.
...
2. **Workers launch and run on their own.** Confirm each worker sub-agent still starts and completes a task — a worker that silently fails to start usually means its config got mangled during install (see Host notes). Workers don't use Monet — on hosts that support a per-subagent tool denylist (Claude Code: `disallowedTools: mcp__monet`), this is enforced at config level for every worker whose roster entry lacks `touchesMonet: true`; on hosts without a denylist mechanism, it is a behavioral guarantee (role prompts never call memory tools). On Codex, workers inherit the parent's Monet server and can see its tools but don't use them — this is expected and cannot be prevented at config level (see Host notes). (Lead-only installs: skip the worker check — instead verify the Stig block with its `with-monet:mode` marker is present at the persona surface, and after the reload confirm the session actually opens as Stig: it should identify as the product owner and reach for `agent_context`, not behave as the host's default persona. A passing Monet check alone doesn't prove the persona loaded.)
3. **Git/GitHub operations require explicit authorization.** Verify that the installed Stig's persona keeps drafting a contract, creating an issue, and implementing it as separate authorization boundaries; git commit, push, PR open/reply/merge operations likewise require the user's explicit go-ahead per-action or as a standing First Block instruction. Without authorization, Stig prepares the contract, change, or exact commands without performing the state-changing action. This invariant must survive every upgrade.
4. **Product ownership and all three curves survive install and update.** Read back the installed Stig block and confirm PRODUCT (what matters), TECHNICAL (what is true/feasible for this system, not generic coding), and USER (how collaboration/decisions operate) map to separate project product, project technical/system, and user context families; mature decisions compose them, scoped corrections improve them without leaking another project's boundary, and self-evaluation covers outcome/judgment, system grounding/prior technical outcomes, user fit/autonomy, and smallest adequate response with less burden. Product owner remains primary in team and lead-only modes: workers own technical truth, Stig makes routine disposition, and the user ratifies new direction or meaningful contract/priority/scope changes. Routine technical work skips ceremony; a local `0644` exposure is calibrated against supported boundaries and harm rather than automatically blocking, while credible in-contract integrity/security risk may block; drafting does not authorize issue creation or implementation; explicit supported contracts and safety invariants override preferences; and a technically complete but product-incomplete result is `Product Gap`.
