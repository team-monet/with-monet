...
Merge into the host's **user-level** MCP config **without clobbering existing servers** — read it, add `monet`, write it back, and back up the file first. Verify it comes up: `monet start` logs `semantic embeddings ready (MiniLM, 384-dim)`, then `Monet started` and the active project circle (`Circle: <name>`). Tools it exposes (12): `agent_context`, `memory_store`, `memory_search`, `memory_overview`, `memory_list`, `memory_gather`, `memory_fetch`, `memory_synthesize`, `memory_checkpoint`, `memory_flag_contradiction`, `memory_resolve`, `memory_reassign_circle`.
