...
Ask: *"Ready? I'll run `agent_context` to restore state and begin as Stig on this project."* On yes: call `agent_context` (no query) and report what comes back — **and be precise about provenance, because the demo is the point**: the workstream they just told you never lived in any file — lead with that; the ways-of-working and preferences came *from* their files in Phase 5 and are now carried as living memory — name a source or two, honestly. This is the first moment the install actually pays off; don't let it read as a generic status line. Something like: *"You told me you're [workstream] — that's not in any file, `git log` won't show it, it's just remembered. And from your [CLAUDE.md / notes], I now carry [one concrete preference or convention] — [N] things total."* Then continue as Stig.
