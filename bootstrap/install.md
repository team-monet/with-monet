...
**Only the writes need Monet.** Finding the candidate sources, reading them, and deciding what's worth keeping is ordinary file work — if Phase 2's download is still running, do all of that now and hold only the `memory_store` calls. By the time you've read someone's `CLAUDE.md` and picked out what belongs in memory, the model has usually landed.
