...
- the **First Block offer**: when the user states a rule meant to govern every session, store it and offer to pin it — written as an instruction rather than a label — and never pin without asking,
...
Then build the first **context profile**: how this particular system works and changes safely, what it is trying to achieve, and how this user likes to work. Keep them as separate, scoped concepts rather than one blob; current ticket/task/session state belongs in the tracker or the checkpoint, never as durable context. Capture a coherent high-level concept when a future task needs the whole model, and precise scoped facts when exact recall matters.
...
Verify the context profile like any other capture: `memory_search` / `memory_fetch` the project and user concepts, synthesize if needed, and summarize what Monet now knows. Stig recalls only the subset a task needs, and applies user context to its own collaboration.
...
4. **The installed block still carries its invariants.** Read the installed Stig block back and confirm the pieces an update must never lose: the `agent_context`-at-session-start restore with its never-adopt-a-workstream-silently rule; the `memory_store` write discipline (durable understanding only — not activity, not current-task state, not anything recoverable from code, git, a file, or a test result); the First Block offer with its instruction-not-label rule and never-pin-without-asking; the `with-monet:mode` marker; the git/GitHub authorization boundary; and the voice rule. On a `lead-only` install, confirm the lead-only line is present and that Stig does the work itself rather than looking for workers.
