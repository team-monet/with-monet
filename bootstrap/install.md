...
**Storage is one global brain, isolated per project.** The store lives at `~/.monet` and Monet isolates each project into its own *circle* automatically — no environment variables to set, and memory never bleeds between repos.
