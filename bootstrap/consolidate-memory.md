...
- **Monet's own wiring, host-agnostic.** Any file (or section) bearing a `<!-- MONET:STIG -->` or `<!-- MONET:AGENT -->` sentinel — Monet writes one into every lead/worker prompt it installs, wherever the host keeps them (`.claude/agents/`, `.cursor/rules/`, `.continue/rules/`, …). Match the `MONET:STIG` marker by **prefix** (the install writes `<!-- BEGIN MONET:STIG -->` but treat any `<!-- BEGIN MONET:STIG…` as the boundary). Also exclude `.claude/agents/*.md` by path. This is what stops a consolidation pass from capturing/retiring Monet's own agent team and stranding the host.
...
**Every retirement — pointer-stub, archive, or delete — backs up the original first: copy → verify → then modify, never a bare overwrite/`mv`/`rm`:** copy the original to its `.monet-archive/` path, re-read the archived copy and confirm its bytes match, *then* (and only then) overwrite the file with the pointer stub, move it, or delete it. For a delete, additionally re-confirm the content is still fetchable from Monet first. If any step fails, **abort that source and leave the original untouched.** Append a one-line entry to a `.monet-archive/manifest.md` for everything you stub/move/delete, so the whole pass is auditable and reversible.
